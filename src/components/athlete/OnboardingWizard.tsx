...
type Phase = "code-gate" | "intro" | "questionnaire" | "complete";
...
export function OnboardingWizard({ onComplete, onCancel, initialData }: Props) {
  const [phase, setPhase] = useState<Phase>("code-gate");
  ...
  async function handleCodeSubmit() {
    ...
    if (!trimmed) { setCodeError("Bitte gib deinen Code ein."); return; }
    ...
    try {
      ...
      setCodeError("Ein Fehler ist aufgetreten. Bitte versuche es erneut.");
    } finally {
      ...
      <div className="min-h-screen bg-[#0a0f1a] flex items-center justify-center p-6">
        <div className="w-full max-w-sm flex flex-col gap-6">
          <div className="flex flex-col gap-2 text-center">
            <p className="text-2xl font-bold text-[#f0f4ff]">Willkommen</p>
            <p className="text-sm text-[#8fa3c0]">
              Gib bitte deinen persönlichen Code ein, den du von Gustav erhalten hast.
            </p>
          </div>
          <div className="flex flex-col gap-3">
            <input
              ...
              autoFocus
              className="bg-[#0f1624] border border-[#1e2d42] rounded-xl px-4 py-3 text-[#f0f4ff] text-sm text-center tracking-widest focus:outline-none focus:border-[#3b82f6] transition-colors placeholder:text-[#3b4d6a] placeholder:tracking-normal"
            />
            {codeError && (
              <p className="text-xs text-[#ef4444] text-center">{codeError}</p>
            )}
            <button
              onClick={handleCodeSubmit}
              disabled={codeChecking}
              className="flex items-center justify-center gap-2 py-3 rounded-xl bg-[#3b82f6] text-white font-semibold text-sm hover:bg-[#2563eb] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              {codeChecking ? "Wird geprüft…" : "Code bestätigen"}
            </button>
          </div>
          <button
            onClick={onCancel}
